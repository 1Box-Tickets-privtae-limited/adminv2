                                          ...
                                          <tr>
                                             ...
                                             <td>
                                                <div class="form-group mb-1 cust-switch">
                                                   Disable / Enable
                                                   <div class="custom-control custom-switch">
                                                       <input type="checkbox" id="customSwitch018" class="is-switch custom-control-input" name="ignoreautoswitch" value="1" <?php if($event->ignoreautoswitch == '1' || $event->ignoreautoswitch == ''){?> checked <?php } ?> >
...
$('#category').on('change', function(event) {
   ...
   category_id=$(this).val();
    ...
    val = val.replace('-', '') ; // collapse dashes
 var slug ="";
if(val){
    slug = slugfly(val);
    $("#event_url").val(slug);
...
<?php //if(empty($event->m_id)) { ?>
            ...
            if(category_id>0)
            {
               var category_val= $('#category option:selected').text();
               category_val = category_val.replace('-', '')+'-'; 
            }
              

            var val = category_val+$("#eventname").val();
            var slug ="";
            if(val){
               slug = slugfly(val);
