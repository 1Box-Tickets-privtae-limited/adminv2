		...
		foreach($records as $record ){
			...
			<div class="dropdown-menu dropdown-menu-right">
			   ...
			   <a href="#" class="dropdown-item">Edit Email </a>
					 ...
					 if ($record->booking_status == 3) { 
						$delivery_status ='<div class="bttns">
						<span class="badge-danger">Cancelled</span>
					  </div>';
					 } 
					
					 if ($record->booking_status == 4) { 
						$delivery_status ='<div class="bttns">
						<span class="badge bg-info">Shipped</span>
					  </div>';
					 } 
					 if ($record->booking_status == 5) { 
						$delivery_status ='<div class="bttns">
						<span class="badge bg-warning">Delivered</span>
					  </div>';
					 } 
					 if ($record->booking_status == 6) { 
						$delivery_status ='<div class="bttns">
			...
			$data[] = array( 
				"total"				    => '<div class="form-check custom-checkbox"><input type="checkbox" class="form-check-input dt-checkboxes" value="'.$record->bg_id.'"><label class="form-check-label">&nbsp;</label></div>',	
	...
	foreach($records as $record ){
		...
		// exit;
			...
			//}
				...
				if($_SERVER['REMOTE_ADDR'] == "2401:4900:606f:5118:dc77:f20b:a942:efe3"){

					//echo "<pre>";print_r($this->data['orderData']);exit;
				}
...
// exit;
		...
		//	error_reporting(E_ALL);
			...
			if($this->session->userdata('user_token_admin') == ""){
				$this->session->set_userdata('user_token_admin',time().rand(10000000,99999999));
			}
			// ini_set('display_errors', '1');
			// ini_set('display_startup_errors', '1');
			// error_reporting(E_ALL);
			...
			if($match_id){
				...
				$post_data = array("match_id" => $match_id,
								...
								"seller_id"		=> $seller_id,
								"default_currency" => 1,
								'user_token' 	=> $this->session->userdata('user_token_admin')
								);
				//pr($post_data);die;
				$this->load->library('curl');
				$response =  $this->curl->post($url,$post_data);
...
public function send_review_request()
  {
  	if(!empty($_POST['requestIds'])){//echo "<pre>";print_r($_POST);exit;

				$url = API_CRON_URL.'send_review_request';
				$post = array('requestIds' => implode(',', $_POST['requestIds']));
				$handle = curl_init();
				curl_setopt($handle, CURLOPT_URL, $url);
				curl_setopt($handle, CURLOPT_POST, 1);
				curl_setopt($handle, CURLOPT_POSTFIELDS,$post);
				curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);
				$output = curl_exec($handle);print_r($output);exit;
				curl_close($handle);
				 $response['status'] = 1;
	...
	public function get_pending_items()
  {
		...
		if(!empty($_POST['booking_no']) || !empty($_POST['event_name']) || !empty($_POST['status_type']) || !empty($_POST['event_start_date']) || !empty($_POST['event_end_date']) || !empty($_POST['seat']) || !empty($_POST['seller_name']))
		{
			...
			foreach($records as $record ){
				...
				$booking_no='<a href="'.base_url()."game/orders/details/". md5($record->booking_no).'">#'.$record->booking_no.'</a> <br><span class="tr_src_type">'.$record->sold_store_name.'</span>';
