...
$this->data['segment']=$seg;
 ...
 function get_currency($from_currency,$to_currency,$amount,$symbol="")
	...
	public function export(){
		...
		if($segment == "abondaned_cart" || $segment == "failed" || $segment == "success"){
			...
			$this->db->select("booking_global.*, booking_billing_address.first_name,booking_billing_address.last_name,booking_billing_address.email,booking_tickets.match_name,booking_tickets.*,register.first_name as buyer_first_name, register.last_name as buyer_last_name, register.email as buyer_email, 
				...
				,admin_details.admin_name as seller_first_name,admin_details.admin_last_name as seller_last_name,
				partner.admin_name as partner_first_name,partner.admin_last_name as partner_last_name,
				affiliate.admin_name as affiliate_first_name,affiliate.admin_last_name as affiliate_last_name,
				site_settings.site_value as store_name
				"
			...
			if ($_GET['from_date']) {
				$this->db->where('booking_global.created_at >= ', date("Y-m-d 00:00", strtotime($_GET['from_date'])));
			}
			if ($_GET['to_date']) {
				$this->db->where('booking_global.created_at <= ', date("Y-m-d 23:59", strtotime($_GET['to_date'])));
			}
			//$this->db->group_by('')
			$this->db->group_by("booking_global.bg_id");
			//$this->db->limit(10);
            ...
            if($_GET['from_date'] != "" && $_GET['to_date'] != ""){
                $search['from_date']  = $_GET['from_date'];
                $search['to_date']  = $_GET['to_date'];
                $filename = $segment."-" .$_GET['from_date'] . "_To_".$_GET['to_date'].".csv"; 
            }
            else{
                 $filename =  $segment."-" . date('Y-m-d H:i:s') . ".csv"; 
            }
             	//$fp = fopen('file.csv', 'w'); 
				...
				if($count == 0 ){
					$listing_note =  "";
					if($row->listing_note){
						$this->db->select('GROUP_CONCAT(DISTINCT ticket_name separator ", " ) as ticket_names ');
						$this->db->where('language', 'en');
					
						$this->db->where_in('ticket_details_id',explode(',',$row->listing_note));
						$query = $this->db->get('ticket_details_lang');
						$listing_note = $query->row()->ticket_names;
					...
					if(@$email_data[$row->buyer_email] == ""){
		            	...
		            	$lineData = array(
		            		...
		            		$row->affiliate_first_name." ".$row->affiliate_lastname,
		...
		if($segment == "campain_abondaned_cart"){
			$tournment_id = $_GET['tournament'];
			$from_date = $_GET['from_date'];
			$to_date = $_GET['to_date'];
